...
static void report_mmio_overlap(const char *name1, uint32_t l1, paddr_t r1,
    const char *name2, uint32_t l2, paddr_t r2) {
...
/* bus interface */
word_t mmio_read(uint32_t addr, int len) {
  IOMap *target_ptr = fetch_mmio_map(addr);
  word_t ret = map_read(addr, len, target_ptr);
  IFDEF(EN_DTRACE, printf("DTRACE %s read  addr: 0x%8x len: %d data: 0x%08x\n", target_ptr->name, addr, len, ret));
  return ret;
}

void mmio_write(uint32_t addr, int len, word_t data) {
  IOMap *target_ptr = fetch_mmio_map(addr);
  map_write(addr, len, data, target_ptr);
  IFDEF(EN_DTRACE, printf("DTRACE %s write addr: 0x%08x len: %d data: 0x%08x\n", target_ptr->name, addr, len, data));
}
